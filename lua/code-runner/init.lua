...
local function async_run(runner, ...)
	...
	elseif type(runner) == "table" and #runner == 2 then
		target = util.unify_path(vim.fn.tempname(), ":p")
...
local function on_backgroud_exit(id, code, single)
	...
	nt.notify("task finished with code=" .. code .. " in " .. util.trim(vim.fn.reltimestr(end_time)) .. " seconds")
end
...
function M.setup(opt)
	if opt.runners then
		for ft, runner in pairs(opt.runners) do
			runners[ft] = runner
		end
	end
	enter_win = opt.enter
